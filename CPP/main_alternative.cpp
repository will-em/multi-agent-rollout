            ...
            while(!simEnv.isDone() && iteration < horizon){
                ...
                */
                ...
                if(iteration != 0 && simEnvStepCost > 1000.0){
                    ...
                    // Swap agents that are part of collision to the front of the line
                    auto partOfCollision = env.getPartOfCollision();

                    for(size_t i = 0; i < partOfCollision.size(); i++){
                        // Search for agentIdx
                        for(size_t j = 0; j < numOfAgents; j++){
                            if(partOfCollision[i] == agentOrder[j]){
                                // Swap
                                int temp = agentOrder[i];
                                agentOrder[i] = agentOrder[j];
                                agentOrder[j] = temp;
                            }
                        }
                    }
                    ...
                    for(size_t i = 0; i < agentsOnDropOff.size(); i++){
                        // Search for agentIdx
                        for(size_t j = 0; j < numOfAgents; j++){
                            if(agentsOnDropOff[i] == agentOrder[j]){
                                // Swap
                                int temp = agentOrder[numOfAgents - i - 1];
                                agentOrder[numOfAgents - i - 1] = agentOrder[j];
                                agentOrder[j] = temp;
                            }
                        }
                    }
