...
#define GENERATE_TURN_IMAGES false
#define GENERATE_SMOOTH_IMAGES true
...
void simulate(int numOfAgents){ 
    ...
    while(!env.isDone()){
		...
		if (GENERATE_TURN_IMAGES) {
			std::pair<int,int> size(env.getDim(), env.getDim());
			generate_image("video/frames/", current_frame, env.getMatPtr(), &size);
		...
		if (GENERATE_SMOOTH_IMAGES) {
			...
			for (int i=0; i<12; i++) {
				...
				generate_interpolated_image("video/frames/", current_frame, beforeEnv.getMatPtr(), env.getMatPtr(), alpha, &size, numOfAgents);
				current_frame++;
			}
