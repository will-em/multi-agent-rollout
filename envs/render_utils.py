...
def room_to_rgb(room, agents, room_structure=None):
    """
    ...
    if not room_structure is None:
        ...
        for agent in agents:
            room[((room == agent.id) | (room == agent.id+1))
                 & (room_structure == 2)] = 2*len(agents) + agent.id - (agent.id - 5)//2
    ...
    for i in range(len(agents)):
        i_x = i%10
        i_y = i//10
        surfaces.append(player_sprite[res*i_y:res*(i_y+1), res*i_x:(res*(i_x+1)),
                                      :])
        surfaces.append(player_with_box_sprite[res*i_y:res*(1+i_y), res*i_x:(res*(i_x+1)),
                                               :])
    
    for i in range(len(agents)):
        i_x = i%10
        i_y = i//10
        surfaces.append(player_on_target_sprite[res*i_y:res*(1+i_y), res*i_x:(res*(i_x+1)),
                                               :])
...
def room_to_tiny_world_rgb(room, room_structure=None, scale=1):
    ...
    surfaces = [wall, floor, box_target,
                box_on_target, box, player, player_on_target]
    ...
    for i in range(room.shape[0]):
        ...
        for j in range(room.shape[1]):
            ...
            room_small_rgb[x_i:(x_i+scale), y_j:(y_j+scale),
                           :] = np.array(surfaces[surfaces_id])
...
def room_to_rgb_FT(room, box_mapping, room_structure=None):
    ...
    for i in range(room.shape[0]):
        ...
        for j in range(room.shape[1]):
            ...
            if 1 < surfaces_id < 5:
                try:
                    surface = get_proper_box_surface(
                        surfaces_id, box_mapping, i, j)
                except:
                    pass
...
def room_to_tiny_world_rgb_FT(room, box_mapping, room_structure=None, scale=1):
    ...
    for i in range(room.shape[0]):
        ...
        for j in range(room.shape[1]):
            ...
            if 1 < surfaces_id < 5:
                try:
                    surface = get_proper_tiny_box_surface(
                        surfaces_id, box_mapping, i, j)
                except:
                    pass
